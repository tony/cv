...
export const getActivityLanguagePieData = (activities, languageMap, actors) => {
  /**
   * Return activity data structured for a line chart based on activities.
   *
   * @param activities {Array, <Object>}
  ...
  let l = activities.reduce((languages, activity) => {
    ...
    const activityLanguages = actors[activity.actor].languages.map(lang => languageMap[lang]);
