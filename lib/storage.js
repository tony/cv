...
export function availableActivityTypes(activities, activityTypes) {
  return activityTypes.filter((activityType) => (
    Object.values(activities).find(activity => activity.component_name === activityType.component_name)
  ));
}
    ...
    Object.assign(
      ...
      activity,
      {
        actor: Object.assign(
          ...
          {
            languages: actors[activity.actor].languages ? actors[activity.actor].languages.map(language => languages[language]) : []
          }
        ),
      }
    )
...
export const selectVisibleActivities = (selectedLanguages, selectedActors, activities, filteredActivities, languageMap, actorMap) => {
  /**
  ...
  return denormalizeActivities(filteredActivities, actorMap, languageMap);
};
