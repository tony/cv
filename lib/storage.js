...
export function availableActorIds(actors, activities) {
   ...
   */
  return [
    ...new Set(Object.keys(actors).filter(actor => activities.find(activity => activity.actor == actor)))
  ];
}
...
export const selectActivityIds = (activityIds, activities, selectedActivityTypes, selectedFilters) => {
  ...
  selectedFilters.forEach((filterName) => {
    selectedIds = selectedIds.filter((i) => {
      return filters[filterName](activities[i]);
    });
  });
...
export const selectVisibleActivityIds = (selectedLanguages, selectedActors, activities, filteredActivities, languages, actors) => {
  /**
...
export const countLanguagesFromActivities = (activities, languageMap, actorMap) => {
  return activities.reduce((languages, activity) => {
    ...
    const activityLanguages = actorMap[activity.actor].languages.map(lang => languageMap[lang]);
