...
export default {
    ...
    this.renderChart(this.chartData, {
      ...
      onClick: (e, i) => {
        if (i && i.length) {
          ...
          if (
            !this.$store.state.selectedLanguages.find(
              lang => lang.name === language,
            )
          ) {
            this.$store.commit('updateSelectedLanguages', [
              ...this.$store.state.selectedLanguages,
              ...this.$store.getters.availableLanguages.filter(
                lang => lang.name === language,
              ),
            ]);
          }
  ...
  methods: {
    ...mapActions(['updateSelectedLanguagesAction']),
  },
};
